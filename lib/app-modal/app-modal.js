...
class ModalComponent extends DWCKComponent {
	...
	set open(value) {
		...
		if (!checker) {
			// to queue actions if the ui hasn't finish rendeing
			setTimeout(() => {
	...
	async #init() {
		...
		let unregisterTriggers = registerTriggers(this, (event) =>
			this.show(event)
		);
		...
		window.addEventListener("refresh-triggers", (event) => {
			console.info("refreshing triggers");
			unregisterTriggers && unregisterTriggers();
			unregisterTriggers = registerTriggers(this, (event) =>
				this.show(event)
			);
		});
	...
	show(event = {}) {
		...
		Array.from(this.querySelectorAll("[slot]")).forEach((slot) =>
			initializeValues(
				slot,
				new CustomEvent("updata-modal-content", {
					bubbles: false,
					composed: false,
					detail: data,
				})
