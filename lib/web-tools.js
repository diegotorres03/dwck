...
function updateVars(component) {
  ...
  /** @type {HTMLElement[]} */
  const vars = [...component.shadowRoot.querySelectorAll('var')]

  vars.forEach(variable => {
    const varName = variable.getAttribute('key')
    const value = component[varName]
    if(!value) return
    variable.setAttribute('value', value)
    variable.textContent = value
  })

  // legacy, move to upper style with <var id="var name as it appear in code"> tag
  const variables = Array.from(
    component.shadowRoot.querySelectorAll('.variable'), // legacy
  )
  variables.forEach((variable) =>
    Array.from(variable.classList)
      .filter((item) => item !== 'variable')
      .forEach((varName) => {
        console.warn('this will be deprecated, use the <var key="varName"> tag')
        const property = varName.replace('-var', '')
        variable.textContent = component[property]
        console.log('sett', varName, property)
      }),
  )
}
