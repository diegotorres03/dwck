...
export default class DataStore extends DWCKComponent {
  ...
  /**
   * Initialize IndexedDB and tables
   * @private
   * @memberof DataStore
   */
  #dbInit() {
    ...
    Array.from(this.querySelectorAll('data-set')).forEach(dataset => {
      stores[dataset.id] = dataset.hasAttribute('indexes') ? dataset.getAttribute('indexes') : 'id'
  ...
  async #processEvent(event) {
    ...
    // [ ] review the append logic
    // why this is working here if the append is being added to the data-set instead?
    if (event.target.hasAttribute('append') ) {
      ...
      data.id = data.__id + ':' + (totalItems + 1)
      
      return table.add(data)
    }
    
    if (!data.id) data.id = data.name
    return table.put(data, data.id || data.__id)
  ...
  async #processCrudEvent(event, queryType) {
    ...
    if (queryType === list) {
      ...
      this.removeItem(table, id)
        .then(res => console.info('deleted', res))
